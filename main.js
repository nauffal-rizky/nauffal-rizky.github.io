...
window.addEventListener("scroll", () => {
	...
	if (scrollHeight > 500) {
		menuSystem.classList.add("show-link");
	} else {
		menuSystem.classList.remove("show-link");
	}
});
...
const mbs = menuBtn.querySelector("svg");
const mbsl = mbs.querySelectorAll("line");
mbsl.forEach((line) => {
	window.addEventListener("click", (e) => {
		if (e.target == menuBtn || e.target == mbs || e.target == line) {
			menuSystem.classList.add("blur");
		} else {
			menuSystem.classList.remove("blur");
		}
	});
});
